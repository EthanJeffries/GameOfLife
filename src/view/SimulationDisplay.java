...
public class SimulationDisplay extends Application{
	...
	private final int CELL_PRESENT = 1;			// final instance variable for boolean checks if bacteria is present
	...
	private void setupSimulation() {
		...
		this.simulationDrawing.getChildren().add(new Rectangle(this.blockSize*this.numRows, this.blockSize*this.numColumns, Color.WHITE));
	...
	public void redrawSimulation() {
		for(int x = 0; x < this.displayGrid.length; x++) {
			for(int y = 0; y < this.displayGrid[x].length; y++) {
				...
				if (this.simController.getState(y, x)==this.CELL_PRESENT) {
